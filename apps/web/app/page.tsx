...
import { CheckCircle2, Loader2, LogOut, QrCode, ShoppingCart, Ticket, UserRound } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setToken(savedToken);
    loadEvents();
    if (savedToken) loadMyTickets(savedToken);
  }, []);
  ...
  async function loadEvents() {
    try {
      const result = await api<{ events: EventCard[] }>("/api/events");
      setEvents(result.events);
      setSelectedEvent(result.events[0] ?? null);
    } catch {
      setMessage("No se pudieron cargar eventos desde la API.");
    }
  }

  async function loadMyTickets(activeToken = token) {
    if (!activeToken) return;
    try {
      const result = await api<{ tickets: MyTicket[] }>("/api/tickets/mine", {}, activeToken);
      setMyTickets(result.tickets);
    } catch {
      localStorage.removeItem("gen.web.token");
      setToken("");
      setMessage("Tu sesion expiro. Entra otra vez.");
    }
  }
  ...
  async function submitAuth() {
    ...
    if (!cleanEmail.includes("@")) return setMessage("Escribe un correo valido.");
    if (password.length < 8) return setMessage("El password debe tener minimo 8 caracteres.");
    if (mode === "register" && cleanName.length < 2) return setMessage("Escribe tu nombre.");
    ...
    try {
      ...
      setAttendeeName(cleanName || cleanEmail.split("@")[0]);
      await loadMyTickets(result.token);
  ...
  async function buyTicket() {
    if (!token || !selectedEvent || !ticketTypeId) return setMessage("Primero entra y selecciona un boleto.");
    const cleanAttendee = attendeeName.trim();
    if (cleanAttendee.length < 2) return setMessage("Escribe el nombre del asistente.");
    ...
    try {
      const result = await api<IssuedTicket>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ eventId: selectedEvent.id, ticketTypeId, attendeeName: cleanAttendee })
        },
        token
      );
      ...
      await api<Checkout>(
        ...
        { method: "POST", body: JSON.stringify({ ticketId: result.ticket.id, provider: "stripe" }) },
        token
      );
      await loadMyTickets(token);
      setMessage("Boleto comprado y QR generado. Guarda el QR para el acceso.");
    } catch {
      setMessage("No se pudo completar la compra. Intenta de nuevo.");
    } finally {
      setLoading(false);
    }
  }

  async function showTicketQr(ticketId: string) {
    if (!token) return;
    setLoading(true);
    setMessage("");
    try {
      const result = await api<{ qr: IssuedTicket["qr"] & { ticket: { id: string } } }>(`/api/tickets/${ticketId}/qr`, { method: "POST" }, token);
      const ticket = myTickets.find((item) => item.id === ticketId);
      setIssuedTicket({
        ticket: {
          id: ticketId,
          attendee_name: ticket?.attendee_name ?? "Asistente GEN",
          status: ticket?.status ?? "issued",
          price_cents: ticket?.price_cents ?? 0
        },
        qr: result.qr
      });
      setMessage("QR recuperado. Este QR temporal reemplaza el anterior.");
    } catch {
      setMessage("No se pudo recuperar el QR.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#03030a] text-white">
      ...
      <section className="mx-auto grid max-w-6xl gap-5 px-4 py-8 lg:grid-cols-[0.82fr_1.18fr]">
        <aside className="space-y-5">
          <div className="rounded-lg border border-white/10 bg-white/[0.06] p-5">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white/58">Tu sesion queda guardada por 30 dias.</p>
              </div>
            </div>

            {!token ? (
              <div className="space-y-3">
                {mode === "register" ? <input className="w-full rounded-md border border-white/10 bg-black/40 p-3" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre" /> : null}
              ...
              <div className="rounded-md border border-[#c7ff3d]/30 bg-[#c7ff3d]/10 p-4 text-sm text-white/78">
                ...
                Ya puedes comprar boletos y recuperar tus QR desde esta web.
              </div>
            )}
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.06] p-5">
            <h2 className="mb-3 flex items-center gap-2 text-xl font-black"><QrCode className="text-[#00d4ff]" /> Mis boletos</h2>
            {token && myTickets.length > 0 ? (
              <div className="space-y-3">
                {myTickets.map((ticket) => (
                  <button key={ticket.id} onClick={() => showTicketQr(ticket.id)} className="w-full rounded-md border border-white/10 bg-black/30 p-3 text-left">
                    <p className="font-black">{ticket.event_title}</p>
                    <p className="text-sm text-white/58">{ticket.attendee_name} · {ticket.tier} · {money(ticket.price_cents)}</p>
                    <p className="mt-1 text-xs font-bold text-[#00d4ff]">Ver QR</p>
                  </button>
                ))}
              </div>
            ) : (
              <p className="text-sm text-white/58">{token ? "Todavia no tienes boletos." : "Entra para ver tus boletos."}</p>
            )}
          </div>
        </aside>
        ...
        <section className="space-y-5">
          <div className="rounded-lg border border-white/10 bg-white/[0.06] p-5">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-black">Venta de boletos</h2>
                <p className="text-sm text-white/58">Elige evento, boleto y genera tu QR.</p>
              </div>
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              {events.map((event) => (
                <button key={event.id} onClick={() => { setSelectedEvent(event); setIssuedTicket(null); }} className={`overflow-hidden rounded-lg border text-left transition ${selectedEvent?.id === event.id ? "border-[#00d4ff]" : "border-white/10"}`}>
          ...
          {selectedEvent ? (
            <div className="grid gap-5 lg:grid-cols-2">
              <div className="rounded-lg border border-white/10 bg-white/[0.06] p-5">
                <h3 className="flex items-center gap-2 text-xl font-black"><ShoppingCart className="text-[#c7ff3d]" /> Comprar boleto</h3>
                ...
                <select className="mt-4 w-full rounded-md border border-white/10 bg-black p-3" value={ticketTypeId} onChange={(e) => setTicketTypeId(e.target.value)}>
                  {ticketTypes.map((ticket) => <option key={ticket.id} value={ticket.id}>{ticket.name} · {money(ticket.price_cents)}</option>)}
                </select>
                ...
                <button onClick={buyTicket} disabled={loading || !token} className="mt-4 flex w-full items-center justify-center gap-2 rounded-md bg-white p-3 font-black text-black disabled:opacity-40">
                  {loading ? <Loader2 className="animate-spin" size={18} /> : null}
                  Comprar boleto {selectedTicketType ? `· ${money(selectedTicketType.price_cents)}` : ""}
                </button>
                {!token ? <p className="mt-3 text-sm text-white/50">Crea cuenta o entra para comprar.</p> : null}
              </div>

              <div className="rounded-lg border border-white/10 bg-white/[0.06] p-5">
                <h3 className="text-xl font-black">QR de acceso</h3>
                {issuedTicket ? (
                  <div className="mt-4">
                  ...
                  <div className="mt-4 grid h-72 place-items-center rounded-md border border-dashed border-white/15 text-center text-white/55">
                    <div>
                      ...
                      Compra o abre un boleto para ver el QR.
                    </div>
                  </div>
                )}
              </div>
